...
assert.ok(existsSync(path.join(vendorDir, "mortic-canonical-state", "SKILL.md")), "vendored canonical-state skill missing");
...
const canonicalTarget = path.join(targetRoot, "mortic-canonical-state");
const manifestPath = path.join(canonicalTarget, ".mortic-skill-manifest.json");
const skillMdPath = path.join(canonicalTarget, "SKILL.md");
...
try {
  // 1. Fresh target: both skills install.
  let actions = actionsByName(await syncVendoredSkills({ vendorDir, targetRoot }));
  assert.equal(actions.get("mortic-canonical-state"), "installed");
  ...
  assert.equal(actions.get("mortic-canonical-state"), "current");

  // 3. Identical unmanaged copy: manifest removed, content identical -> adopted.
  rmSync(manifestPath);
  actions = actionsByName(await syncVendoredSkills({ vendorDir, targetRoot }));
  assert.equal(actions.get("mortic-canonical-state"), "adopted");
  ...
  actions = actionsByName(await syncVendoredSkills({ vendorDir, targetRoot }));
  assert.equal(actions.get("mortic-canonical-state"), "kept-user-copy");
  ...
  try {
    ...
    const vendorSkillMd = path.join(vendorCopy, "mortic-canonical-state", "SKILL.md");
    ...
    assert.equal(actions.get("mortic-canonical-state"), "upgraded");
