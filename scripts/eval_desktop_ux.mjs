...
const checks = [
  ...
  sourceCheck("canonical-default-off", "Canonical project memory is dormant by default.", source.runtime.includes('process.env.MORTIC_CANONICAL_MEMORY === "1"') && source.server.includes("canonical_memory_disabled"), files.runtime),
  sourceCheck("canonical-ui-retired", "The active client does not call project-memory APIs.", !source.app.includes("/api/project/"), files.app),
