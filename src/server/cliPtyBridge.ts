...
function findMorticRoot(): string {
  // The compiled module lives at <root>/dist/node/server/cliPtyBridge.js, so
  // the package root that ships scripts/codex_pty_worker.py is exactly four
  // levels up. This also holds for packed installs under node_modules.
  const moduleFile = fileURLToPath(import.meta.url);
  const packagedRoot = path.resolve(moduleFile, "../../../..");
  if (existsSync(path.join(packagedRoot, "scripts", "codex_pty_worker.py"))) {
    return packagedRoot;
  }

  // Fallback for the dev tree (tsx runs src/server directly) and unexpected
  // layouts: walk upward until a root that contains the PTY worker appears.
...
// The PTY fallback shells out to python3 with POSIX pty support. Where that is
// impossible (win32, or python3 missing from PATH) the fallback must disable
// itself with one readable console message — never an unhandled "error" event,
// which would crash the whole server with a stack trace.
let ptyDisabledNoticeShown = false;
function notePtyFallbackDisabled(reason: string): void {
  if (ptyDisabledNoticeShown) return;
  ptyDisabledNoticeShown = true;
  console.warn(`Codex CLI PTY fallback disabled: ${reason}`);
}
...
export class CodexCliPtyBridge {
  ...
  private start(
    ...
    if (process.platform === "win32") {
      const reason = "the PTY worker needs python3 with POSIX pty support, which Windows does not provide";
      notePtyFallbackDisabled(reason);
      return Promise.reject(new Error(`Codex CLI PTY fallback unavailable: ${reason}`));
    }
    ...
    // A missing python3 makes spawn emit "error" (ENOENT) instead of "exit";
    // without a listener that event is an uncaught exception. Convert it to a
    // single console notice plus a clean rejection of the ready promise.
    child.on("error", (error: Error) => {
      const code = (error as NodeJS.ErrnoException).code;
      const reason = code === "ENOENT" ? "python3 was not found on PATH (the PTY worker is POSIX only)" : error.message;
      notePtyFallbackDisabled(reason);
      child.emit("mortic-worker-spawn-error", new Error(`Codex CLI PTY fallback unavailable: ${reason}`));
    });
    child.stdin.on("error", () => {
      // Writes can race a worker that already died; turn failures surface via
      // the exit/error paths instead of an uncaught stream exception.
    });
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        ...
        child.off("mortic-worker-spawn-error", onSpawnError);
      ...
      child.once("mortic-worker-spawn-error", onSpawnError);
      timeout.unref?.();
    });
  }
