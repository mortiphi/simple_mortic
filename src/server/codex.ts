...
export async function getCodexStatus(): Promise<CodexStatus> {
  try {
    const pathResult = await runCommand("which", ["codex"], undefined, 5000);
    ...
    const versionResult = await runCommand("codex", ["--version"], undefined, 5000);
    return {
...
export async function runCodexTurn(params: {
  ...
  return runCodexEphemeralTurn(params);
}
