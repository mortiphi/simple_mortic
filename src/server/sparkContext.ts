...
// Preflight runs on every non-default-model turn and on every client
// settings change, but the underlying JSONL telemetry only changes when a
// Codex turn finishes. A short listing TTL plus an mtime-keyed parse memo
// keeps repeated preflights off the session-directory walk and file reads.
const SESSION_SCAN_TTL_MS = 10_000;
const sessionFilesCache = new Map<string, { at: number; candidates: SessionFileCandidate[] }>();
const fileTokenCountCache = new Map<string, { mtimeMs: number; value: TokenCountCandidate | null }>();
const FILE_TOKEN_CACHE_MAX = 2048;

async function findSessionFiles(threadId: string): Promise<SessionFileCandidate[]> {
  const cached = sessionFilesCache.get(threadId);
  if (cached && Date.now() - cached.at < SESSION_SCAN_TTL_MS) {
  ...
  sessionFilesCache.set(threadId, { at: Date.now(), candidates: sorted });
  return sorted;
}
...
async function cachedTokenCountFromFile(candidate: SessionFileCandidate): Promise<TokenCountCandidate | null> {
  const cached = fileTokenCountCache.get(candidate.file);
  if (cached && cached.mtimeMs === candidate.mtimeMs) {
    return cached.value;
  }
  const value = await latestTokenCountFromFile(candidate.file);
  if (fileTokenCountCache.size >= FILE_TOKEN_CACHE_MAX) {
    fileTokenCountCache.clear();
  }
  fileTokenCountCache.set(candidate.file, { mtimeMs: candidate.mtimeMs, value });
  return value;
}
...
export async function latestThreadTokenCount(threadId: string): Promise<TokenCountCandidate | null> {
  ...
  for (const candidate of sessionFiles) {
    const tokenCount = await cachedTokenCountFromFile(candidate);
